...
ssh -i %KEY_PATH% opc@%VPS_IP% "sudo pm2 kill && sudo fuser -k 8080/tcp 2>/dev/null && sudo rm -rf /root/.pm2/dump.pm2 && sudo ADMIN_KEY=flappyAdmin2026 pm2 start /home/opc/server.js --name 'flappy' && sudo pm2 save"

echo ----------------------------------------------------
echo DEPLOY SUCCESSFUL! PORT 8080 IS NOW CLEAN.
echo Opening Live Logs... (Press Ctrl+C to stop logging)
echo ----------------------------------------------------

ssh -i %KEY_PATH% opc@%VPS_IP% "sudo pm2 logs flappy"
pause
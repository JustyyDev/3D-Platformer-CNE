...
function create() {
    ...
    platformCX = FlxG.width / 2; platformCY = FlxG.height / 2 + 20;

    FlxG.camera.bgColor = 0xFF080816;
    perspShader = new CustomShader("perspective");
    perspShader.skew = 0.15;
    perspShader.depth = 0.6;
    perspShader.tilt = 0.25;
    ...
    platformBorder.alpha = 0.4; platformBorder.antialiasing = true;
...
function goToState(s:String) {
    ...
    switch(s) {
        case "MENU": lobbyText.text = "[1] SOLO (VS BOTS)   [2] MULTI   [ESC] BACK"; FlxG.sound.playMusic(Paths.music("flappy/mainTheme"), 0.8, true);
...
function startRound() {
    ...
    playerVX = 0; playerVY = 0; dashCooldown = 0;
...
function spawnBots(count:Int) {
    ...
    for (i in 0...count) {
        ...
        var bot = new FlxSprite(bx, by).makeGraphic(32, 26, col); bot.alpha = 0.8; playerGroup.add(bot);
        var botE = new FlxSprite(0, 0).makeGraphic(8, 8, 0xFFFFFFFF); botE.alpha = 0.8; playerGroup.add(botE); Reflect.setField(playerEyeMap, nick, botE);
        var botB = new FlxSprite(0, 0).makeGraphic(12, 7, 0xFFFF8800); botB.alpha = 0.8; playerGroup.add(botB); Reflect.setField(playerBeakMap, nick, botB);
        ...
        Reflect.setField(playerVXMap, nick, 0.0); Reflect.setField(playerVYMap, nick, 0.0);
...
function updateBotAI(elapsed:Float) {
    for (i in 0...activePlayers.length) {
        ...
        var bvy:Float = Reflect.field(playerVYMap, nick); if (bvy == null) bvy = 0;
...
function spawnFallVFX(x:Float, y:Float) {
    for (i in 0...8) {
        ...
        p.alpha = 0.8; p.velocity.set(FlxG.random.float(-150, 150), FlxG.random.float(-180, 80));
        vfxGroup.add(p);
...
function showGameOver(won:Bool, reward:Int) {
    ...
    var result = won ? "LAST BIRD STANDING!" : "KNOCKED OUT!";
...
function updatePlaying(elapsed:Float) {
    ...
    if (platformShrinkTimer >= platformShrinkInterval && platformRadius > platformMinRadius) {
        ...
        shrinkWarning.visible = true; shrinkWarning.text = "PLATFORM SHRINKING!"; shrinkWarning.alpha = 1;
    ...
    if (!iAmDead) {
        ...
        for (i in 0...activePlayers.length) {
            ...
            if (Reflect.field(deadMap, nick)) continue;
            var op:FlxSprite = Reflect.field(playerMap, nick); if (op == null) continue;
            var ovx:Float = Reflect.field(playerVXMap, nick); if (ovx == null) ovx = 0;
            ...
            if (result.hit) {
                playerVX = result.avx; playerVY = result.avy;
                ...
                if (isMultiplayer) netSend("BUMP:" + nick + ":" + Std.int(result.bvx) + ":" + Std.int(result.bvy));
            }
        }
        ...
        if (isMultiplayer) {
            ...
            if (netSendAccum >= 0.05) { netSendAccum = 0; netSend("POS:" + Std.int(player.x) + ":" + Std.int(player.y)); }
        }
    }
...
function checkPlayerFallOff() {
    var px = player.x + 16; var py = player.y + 13;
...
function spawnDashVFX() {
    for (i in 0...5) {
        ...
        p.alpha = 0.5; p.velocity.set(-playerVX * 0.3 + FlxG.random.float(-40, 40), -playerVY * 0.3 + FlxG.random.float(-40, 40));
        vfxGroup.add(p);
...
function spawnBumpVFX(x:Float, y:Float) {
    for (i in 0...4) {
        ...
        p.alpha = 0.6; p.velocity.set(FlxG.random.float(-120, 120), FlxG.random.float(-120, 120));
        vfxGroup.add(p);
...
function handleServerMessage(cmd:String, args:Array<String>) {
    switch(cmd) {
        ...
        case "POS":
            if (args.length >= 3) { var nick = args[2]; Reflect.setField(targetXMap, nick, Std.parseFloat(args[0])); Reflect.setField(targetYMap, nick, Std.parseFloat(args[1])); getOpponent(nick); }
        case "BUMP":
            if (args.length >= 3) {
                var nick = args[0]; if (nick == myNickname) { playerVX += Std.parseFloat(args[1]); playerVY += Std.parseFloat(args[2]); FlxG.camera.shake(0.01, 0.1); }
            }
        case "DEAD": if (args.length >= 1) { Reflect.setField(deadMap, args[0], true); var op = Reflect.field(playerMap, args[0]); if (op != null) { op.color = 0xFF444444; spawnFallVFX(op.x, op.y); } checkRoundEnd(); }
        case "PLAYER_LIST":
...
function getOpponent(nick:String):FlxSprite {
    ...
    if (activePlayers.indexOf(nick) == -1) {
        ...
        var op = new FlxSprite(platformCX, platformCY).makeGraphic(32, 26, col); op.alpha = 0.6; playerGroup.add(op);
        var opE = new FlxSprite(0, 0).makeGraphic(8, 8, 0xFFFFFFFF); opE.alpha = 0.6; playerGroup.add(opE); Reflect.setField(playerEyeMap, nick, opE);
        var opB = new FlxSprite(0, 0).makeGraphic(12, 7, 0xFFFF8800); opB.alpha = 0.6; playerGroup.add(opB); Reflect.setField(playerBeakMap, nick, opB);
        var tag = new FlxText(0, 0, 150, nick, 11); tag.setFormat(Paths.font(currentFont), 11, col, "center", 1, 0xFF000000); tag.cameras = [uiCam]; tag.alpha = 0.6; nickTagGroup.add(tag); Reflect.setField(playerNickMap, nick, tag);
        Reflect.setField(playerMap, nick, op); Reflect.setField(deadMap, nick, false); Reflect.setField(playerVXMap, nick, 0.0); Reflect.setField(playerVYMap, nick, 0.0);
...
function refreshLobbyUI() {
    ...
    lobbyRoomText.visible = true; lobbyRoomText.text = "ROOM: " + activeRoomCode + "   |   BUMPER BIRDS"; lobbyRoomText.y = 20;

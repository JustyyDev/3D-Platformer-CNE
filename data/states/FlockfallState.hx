...
function create() {
    ...
    skyFar = new FlxBackdrop(Paths.image('menus/flappy/sky'), 0x01, 0, 0); skyFar.scale.set(2.5, 2.5); skyFar.y = FlxG.height - 350;
    sky = new FlxBackdrop(Paths.image('menus/flappy/sky'), 0x01, 0, 0); sky.scale.set(2, 2); sky.y = FlxG.height - 300;
    add(skyFar); add(sky);
...
function saveToLeaderboard(scoreVal:Int, multi:Bool) {
    ...
    if (multi) FlxG.save.data.flappyLbMulti = targetArray; else FlxG.save.data.flappyLbSolo = targetArray;
    FlxG.save.flush();
}
...
function applyBGTheme() {
    ...
    spawnGrassTufts(); bgPropGroup.clear(); for (i in 0...12) spawnBGProp(true);
}
...
function goToState(s:String) {
    gameState = s; typedInput = ""; typingText.text = ""; leaderboardGroup.clear(); lobbySlotGroup.clear(); lobbyBgGroup.clear(); playerSidebar.clear(); shopUIGroup.clear(); shopTextGroup.clear(); plazaBubbleGroup.clear(); lobbyRoomText.visible = false; slingLine.visible = false;
    ...
    typingText.visible = (s == "NICKNAME" || s == "ROOM_INPUT" || s == "PLAZA_TYPE"); typingBg.visible = typingText.visible;
    scoreText.visible = false; scoreShadow.visible = false; levelText.visible = false; coinText.visible = true; coinIconText.visible = true; vignette.alpha = 0;

    var showBird = (s == "PLAYING" || s == "SLINGSHOT" || s == "DEAD" || s == "PLAZA" || s == "PLAZA_TYPE");
    bird.visible = showBird; birdWing.visible = showBird; birdEye.visible = showBird; birdBeak.visible = showBird; birdHat1.visible = showBird; birdHat2.visible = showBird; birdNickTag.visible = showBird;

    switch(s) {
        case "NICKNAME": lobbyText.text = "CHOOSE YOUR NAME"; playMusicTrack("mainTheme");
        case "MENU": lobbyText.text = "[1] FLOCKFALL   [2] SLINGSHOT   [3] MULTI   [4] SCORES   [5] SHOP   [6] PLAZA   [ESC] EXIT"; playMusicTrack("mainTheme");
        case "ROOM_INPUT": lobbyText.text = "ENTER 4-LETTER ROOM CODE"; playMusicTrack("mainTheme");
        case "LEADERBOARD": lobbyText.text = ""; lbCategory = 0; lbScroll = 0; renderLeaderboardLocal(); playMusicTrack("mainTheme");
        case "SHOP": lobbyText.text = ""; if (shopCategory == 3) shopCategory = 0; shopScroll = 0; shopCursor = 0; renderShop(); playMusicTrack("shopTheme");
        case "SHOP_VERIFY":
            lobbyText.text = ""; shopUIGroup.clear(); shopTextGroup.clear();
        ...
        case "PLAZA":
            lobbyText.text = "JOINING PLAZA..."; bird.x = FlxG.width / 2; bird.y = FlxG.height - 80; netConnect("PLAZA", myNickname); playMusicTrack("shopTheme");
    }
}

function resetMultiplayerData() { 
    playerMap = {}; playerWingMap = {}; playerEyeMap = {}; playerBeakMap = {}; playerHat1Map = {}; playerHat2Map = {}; playerNickMap = {}; playerSkinIdMap = {};
    scoreMap = {}; deadMap = {}; emoteMap = {}; targetYMap = {}; activePlayers = []; currentLeader = myNickname; plazaBubbleMap = {}; plazaTargetXMap = {};
}
...
function renderShop() {
    ...
    if (previewTrailIdLocal != "none") {
        ...
        if (previewTrailIdLocal == "rainbow") tCol = 0xFFFF00FF; if (previewTrailIdLocal == "glitch") tCol = 0xFF00FFCC; if (previewTrailIdLocal == "snow") tCol = 0xFFFFFFFF; if (previewTrailIdLocal == "hearts") tCol = 0xFFFF4488; if (previewTrailIdLocal == "toxic") tCol = 0xFF44FF00;
    ...
    applyHat(previewSkinData.hat, hat1, hat2, prevBirdX, prevBirdY, 0, 1.0); hat1.cameras = [uiCam]; hat2.cameras = [uiCam]; shopUIGroup.add(hat1); shopUIGroup.add(hat2);
...
function shopInteract(idx:Int) {
    ...
    if (item.owned) {
        if (shopCategory == 0) { equippedSkinId = item.id; bird.color = item.color; }
...
function netConnect(roomCode:String, nickname:String) { 
    ...
    try { 
        ...
        netConnected = false; lobbyText.text = "CONNECTION FAILED!"; new FlxTimer().start(2, function(t) { goToState("MENU"); });
    } 
}
        ...
        while (c < 20) { 
            ...
            try { 
                ...
                for (si in 0...sublines.length) { 
                    var sub = StringTools.trim(sublines[si]); if (sub.length > 0) processNetLine(sub); 
                } 
...
function handleServerMessage(cmd:String, args:Array<String>) {
    switch(cmd) {
        case "WAITING_FOR_HOST": gameState = "LOBBY"; playMusicTrack("mainTheme"); addLobbyPlayer(myNickname); refreshLobbyUI();
        ...
        case "MODE_CHANGED": if (args.length > 0) { gameMode = Std.parseInt(args[0]); refreshLobbyUI(); }
        case "START": if (args.length > 0) FlxG.random.initialSeed = Std.parseInt(args[0]); if (args.length > 1) tagger = args[1]; beginMultiplayer();
        ...
        case "DEAD": 
            if (args.length >= 1) { 
                var nick = args[0]; Reflect.setField(deadMap, nick, true); var op = getOpponent(nick); 
                if (op != null) { op.color = 0xFF444444; op.acceleration.y = 0; spawnDeathBurst(op); } 
                refreshPlayerSidebar(); checkBattleRoyaleEnd(); 
            }
        case "PLAYER_LIST": 
            if (args.length > 0) { 
                var oldList = lobbyPlayers.join(","); lobbyPlayers = [];
                for (pi in 0...args.length) { var pn = StringTools.trim(args[pi]); if (pn.length > 0) lobbyPlayers.push(pn); } 
                sortLobbyPlayers(); if (lobbyPlayers.length > 0 && lobbyPlayers[0] == myNickname) isHost = true; else isHost = false;
        ...
        case "SKIN": if (args.length >= 2) { var nick = args[0]; var skinId = args[1]; Reflect.setField(playerSkinIdMap, nick, skinId); }
        ...
        case "PLAZA_MOVE":
            if (gameState == "PLAZA" || gameState == "PLAZA_TYPE") {
                var nick = args[0]; Reflect.setField(plazaTargetXMap, nick, Std.parseFloat(args[1])); Reflect.setField(targetYMap, nick, Std.parseFloat(args[2])); getOpponent(nick);
            }
        ...
        case "PLAZA_LEAVE":
            if (activePlayers.indexOf(args[0]) != -1) {
                activePlayers.remove(args[0]); var op = Reflect.field(playerMap, args[0]); if (op != null) { op.kill(); Reflect.field(playerNickMap, args[0]).kill(); }
            }
        case "COIN_UPDATE": addCoins(Std.parseInt(args[0])); FlxG.camera.flash(0x3300FF00, 0.4);
        ...
        case "VERIFY_FAIL": if (gameState == "SHOP_VERIFY") { FlxG.camera.flash(0x33FF0000, 0.4); lobbyText.text = "VERIFICATION FAILED!"; new FlxTimer().start(2, function(t) { goToState("SHOP"); pendingPack = null; }); }
    }
}
...
function refreshPlayerSidebar() {
    ...
    var startY = 110; var slotH = 30; var panelH = Std.int(40 + lobbyPlayers.length * slotH);
    ...
    for (si in 0...lobbyPlayers.length) {
        ...
        var slot = new FlxText(FlxG.width - 218, startY + (si * slotH), 210, label, 16); slot.setFormat(Paths.font(currentFont), 16, isDead ? 0xFF555555 : (isMe ? 0xFFFFFFFF : col), "right", 1, 0xFF000000); slot.alpha = isDead ? 0.4 : (isMe ? 1.0 : 0.75); slot.cameras = [uiCam]; playerSidebar.add(slot);
    }
}
...
function refreshLobbyUI() {
    ...
    lobbyRoomText.visible = true; lobbyRoomText.text = "ROOM: " + activeRoomCode; lobbyRoomText.y = 20;
    
    var countText = new FlxText(0, 50, FlxG.width, lobbyPlayers.length + " / 6 PLAYERS", 18); countText.setFormat(Paths.font(currentFont), 18, 0xFF88CCFF, "center", 1, 0xFF000000); countText.cameras = [uiCam]; lobbySlotGroup.add(countText);

    var startY = FlxG.height * 0.22; var slotH = 56;
    for (si in 0...6) {
        var slotY = startY + (si * slotH); var hasPlayer = si < lobbyPlayers.length; var nick = hasPlayer ? lobbyPlayers[si] : "WAITING..."; var col = hasPlayer ? playerColors[si % playerColors.length] : 0xFF333333; var isMe = hasPlayer && lobbyPlayers[si] == myNickname;
        var cardBg = new FlxSprite(Std.int(FlxG.width * 0.12), Std.int(slotY)).makeGraphic(Std.int(FlxG.width * 0.76), Std.int(slotH - 6), hasPlayer ? 0xFF1A1A2E : 0xFF0D0D18); cardBg.alpha = hasPlayer ? 0.65 : 0.3; cardBg.cameras = [uiCam]; lobbyBgGroup.add(cardBg);
        ...
        var numText = new FlxText(Std.int(FlxG.width * 0.15), slotY + 10, 40, "" + (si + 1), 28); numText.setFormat(Paths.font(currentFont), 28, hasPlayer ? col : 0xFF333333, "center", 2, 0xFF000000); numText.cameras = [uiCam]; lobbySlotGroup.add(numText);
        var nameStr = hasPlayer ? nick.toUpperCase() : "---"; if (isMe) nameStr = nameStr + "  (YOU)";
        var nameText = new FlxText(Std.int(FlxG.width * 0.25), slotY + 12, Std.int(FlxG.width * 0.5), nameStr, 24); nameText.setFormat(Paths.font(currentFont), 24, isMe ? 0xFFFFFFFF : col, "left", 2, 0xFF000000); nameText.cameras = [uiCam];
    ...
    var modeStr = gameModes[gameMode]; var ctrlStr = isHost ? "   |   <- " + modeStr + " ->" : "   |   " + modeStr;
...
function getOpponent(nick:String):FlxSprite {
    if (activePlayers.indexOf(nick) == -1) {
        activePlayers.push(nick); addLobbyPlayer(nick); var col = getPlayerColor(nick);
        ...
        Reflect.setField(playerMap, nick, op); Reflect.setField(scoreMap, nick, 0); Reflect.setField(deadMap, nick, false); Reflect.setField(targetYMap, nick, FlxG.height / 2); Reflect.setField(playerSkinIdMap, nick, "default");
...
function interpolateOpponents(elapsed:Float) { 
    for (i in 0...activePlayers.length) { 
        var nick = activePlayers[i]; if (Reflect.field(deadMap, nick)) continue;
        var op:FlxSprite = Reflect.field(playerMap, nick); var targetY = Reflect.field(targetYMap, nick); 
        if (op == null || targetY == null) continue;
        var diff = targetY - op.y; if (Math.abs(diff) > 200) op.y = targetY; else op.y = FlxMath.lerp(op.y, targetY, elapsed * 12.0); 
        if (gameMode == 3 && tagger == nick) { if (!iAmDead && FlxG.overlap(bird, op)) killBird(); }
    } 
}

function updateCrown() { 
    var highest = score; var leader = myNickname;
    for (i in 0...activePlayers.length) { var nick = activePlayers[i]; var s = Reflect.field(scoreMap, nick); if (!Reflect.field(deadMap, nick) && s > highest) { highest = s; leader = nick; } } 
...
function updatePlaza(elapsed:Float) {
    if (FlxG.keys.justPressed.ESCAPE) goToState("MENU");
    if (gameState == "PLAZA" && FlxG.keys.justPressed.T) { gameState = "PLAZA_TYPE"; typedInput = ""; typingText.visible = true; typingBg.visible = true; }
    ...
    if (gameState == "PLAZA") {
        ...
        netSendAccum += elapsed; if (netSendAccum > 0.1) { netSendAccum = 0; netSend("PLAZA_MOVE:" + bird.x + ":" + bird.y); netSend("CHECK_COINS"); }
    }
    
    var bTxt = Reflect.field(plazaBubbleMap, myNickname); if (bTxt != null) bTxt.setPosition(bird.x - 80, bird.y - 40);
    ...
    for (i in 0...activePlayers.length) {
        ...
        if (op != null && tx != null && ty != null) { op.x = FlxMath.lerp(op.x, tx, elapsed * 10); op.y = FlxMath.lerp(op.y, ty, elapsed * 10); }
        var otx = Reflect.field(plazaBubbleMap, nick); if (otx != null && op != null) otx.setPosition(op.x - 80, op.y - 40);
        
        var opW:FlxSprite = Reflect.field(playerWingMap, nick); var opE:FlxSprite = Reflect.field(playerEyeMap, nick); var opB:FlxSprite = Reflect.field(playerBeakMap, nick); var opN:FlxText = Reflect.field(playerNickMap, nick); var oh1:FlxSprite = Reflect.field(playerHat1Map, nick); var oh2:FlxSprite = Reflect.field(playerHat2Map, nick);
        if (opW != null) { opW.setPosition(op.x - 4, op.y + 14); } if (opE != null) opE.setPosition(op.x + 24, op.y + 6); if (opB != null) opB.setPosition(op.x + 30, op.y + 12); if (opN != null) opN.setPosition(op.x - 80, op.y - 22);
...
function update(elapsed:Float) {
    ...
    cloudSpawnTimer += elapsed; if (cloudSpawnTimer > 3.0) { cloudSpawnTimer = 0; spawnCloud(false); } cloudGroup.forEachAlive(function(c:FlxSprite) { if (c.x + c.width < -50) c.kill(); });
    
    if ((gameState == "PLAYING" || gameState == "SLINGSHOT") && !iAmDead) { wingFlapTimer += elapsed; if (wingFlapTimer > 0.12) { wingFlapTimer = 0; wingUp = !wingUp; } }
    
    if (birdSquash != 1.0) birdSquash = FlxMath.lerp(birdSquash, 1.0, elapsed * 10);
    if (bird.visible && gameState != "PLAZA" && gameState != "PLAZA_TYPE") { 
        bird.scale.set(1.0, birdSquash); birdWing.setPosition(bird.x - 4, bird.y + (wingUp ? 4 : 14)); birdWing.scale.set(1.0, wingUp ? 0.7 : 1.2); birdEye.setPosition(bird.x + 24, bird.y + 6); birdBeak.setPosition(bird.x + 34, bird.y + 12); birdNickTag.setPosition(bird.x - 80, bird.y - 22); 
        ...
        bird.scale.set(1.0, 1.0); birdWing.setPosition(bird.x - 4, bird.y + 14); birdWing.scale.set(1.0, 1.0); birdEye.setPosition(bird.x + 24, bird.y + 6); birdBeak.setPosition(bird.x + 34, bird.y + 12); birdNickTag.setPosition(bird.x - 80, bird.y - 22);
        var sDat = getSkinData(equippedSkinId); birdWing.color = darkenColor(sDat.color, 0.7); applyHat(sDat.hat, birdHat1, birdHat2, bird.x, bird.y, bird.angle, 1.0);
    }
    
    if (gameState != "PLAZA" && gameState != "PLAZA_TYPE") {
        for (i in 0...activePlayers.length) { 
            var nick = activePlayers[i]; var op:FlxSprite = Reflect.field(playerMap, nick); if (op == null) continue; var opW:FlxSprite = Reflect.field(playerWingMap, nick); var opE:FlxSprite = Reflect.field(playerEyeMap, nick); var opB:FlxSprite = Reflect.field(playerBeakMap, nick); var opN:FlxText = Reflect.field(playerNickMap, nick); var oh1:FlxSprite = Reflect.field(playerHat1Map, nick); var oh2:FlxSprite = Reflect.field(playerHat2Map, nick);
            if (opW != null) { opW.setPosition(op.x - 4, op.y + (wingUp ? 4 : 14)); opW.scale.set(1.0, wingUp ? 0.7 : 1.2); } if (opE != null) opE.setPosition(op.x + 24, op.y + 6); if (opB != null) opB.setPosition(op.x + 30, op.y + 12); if (opN != null) opN.setPosition(op.x - 80, op.y - 22);
            var oSid = Reflect.field(playerSkinIdMap, nick); if (oSid != null && oh1 != null && oh2 != null) { applyHat(getSkinData(oSid).hat, oh1, oh2, op.x, op.y, op.angle, 1.0); }
        }
    ...
    if (bgPropSpawnTimer > 1.5) { bgPropSpawnTimer = FlxG.random.float(0, 0.5); if ((gameState == "PLAYING" || gameState == "SLINGSHOT") && !inCountdown) spawnBGProp(); }
    bgPropGroup.forEachAlive(function(prop) { prop.velocity.x = pipeSpeed * 0.3; if (prop.x + prop.width < -50) prop.kill(); });
...
function updateNickname(elapsed:Float) { 
    ...
    if (FlxG.keys.justPressed.ENTER && typedInput.length > 1) { myNickname = typedInput; FlxG.save.data.flappyNickname = myNickname; FlxG.save.flush(); goToState("MENU"); } 
}
...
function updateRoomInput(elapsed:Float) { 
    ...
    if (FlxG.keys.justPressed.ENTER && typedInput.length == 4) { activeRoomCode = typedInput; lobbyText.text = "JOINING..."; netConnect(activeRoomCode, myNickname); isMultiplayer = true; gameState = "WAITING_FOR_OPPONENT"; playMusicTrack("mainTheme"); } 
    if (FlxG.keys.justPressed.ESCAPE) goToState("MENU"); 
}
...
function updateLobby() { 
    if (isHost) {
        if (FlxG.keys.justPressed.LEFT) { gameMode--; if (gameMode < 0) gameMode = gameModes.length - 1; netSend("CHAT:SET_MODE:" + gameMode); refreshLobbyUI(); }
        if (FlxG.keys.justPressed.RIGHT) { gameMode++; if (gameMode >= gameModes.length) gameMode = 0; netSend("CHAT:SET_MODE:" + gameMode); refreshLobbyUI(); }
        if (FlxG.keys.justPressed.ENTER) { if (lobbyPlayers.length >= 2) { netSend("START_GAME"); } else { FlxG.camera.shake(0.01, 0.2); } }
    }
    if (FlxG.keys.justPressed.ESCAPE) goToState("MENU"); 
}
...
function updateLeaderboard() { 
    if (FlxG.keys.justPressed.LEFT || FlxG.keys.justPressed.RIGHT) { lbCategory = (lbCategory == 0) ? 1 : 0; lbScroll = 0; renderLeaderboardLocal(); }
    if (FlxG.keys.justPressed.UP && lbScroll > 0) { lbScroll -= 5; if (lbScroll < 0) lbScroll = 0; renderLeaderboardLocal(); }
    if (FlxG.keys.justPressed.DOWN) { lbScroll += 5; renderLeaderboardLocal(); }
    if (FlxG.keys.justPressed.R) { lbSolo = []; lbMulti = []; FlxG.save.data.flappyLbSolo = lbSolo; FlxG.save.data.flappyLbMulti = lbMulti; FlxG.save.flush(); renderLeaderboardLocal(); }
    if (FlxG.keys.justPressed.ESCAPE) goToState("MENU"); 
}
...
function updatePlaying(elapsed:Float) {
    ...
    if (!iAmDead) {
        if (FlxG.keys.justPressed.SPACE || FlxG.keys.justPressed.UP) { bird.velocity.y = jumpForce; birdSquash = 0.7; spawnVFX(bird, getSkinData(equippedSkinId).color); if (isMultiplayer) netSend("JUMP"); }
        var skData = getSkinData(equippedSkinId); if (FlxG.keys.justPressed.SHIFT && !powerUsed && skData.power != "none") { powerUsed = true; activatePower(skData.power); }
        if (FlxG.keys.justPressed.Q) sendEmote(":)"); if (FlxG.keys.justPressed.W) sendEmote("GG"); if (FlxG.keys.justPressed.E) sendEmote("RIP");
        
        var targetAngle = (bird.velocity.y < 0) ? -20 : 50; if (gameMode == 6) targetAngle = (bird.velocity.y > 0) ? -20 : 50;
        bird.angle = FlxMath.lerp(bird.angle, targetAngle, elapsed * 10); birdWing.angle = bird.angle; birdEye.angle = bird.angle * 0.3; birdBeak.angle = bird.angle * 0.5;
    ...
    for (i in 0...activePlayers.length) { 
        var nick = activePlayers[i]; if (!Reflect.field(deadMap, nick)) { var op = Reflect.field(playerMap, nick); if (op != null) { var tgtAng = (op.velocity.y < 0) ? -20 : 50; if (gameMode == 6) tgtAng = (op.velocity.y > 0) ? -20 : 50; op.angle = FlxMath.lerp(op.angle, tgtAng, elapsed * 10); } } 
    }
    ...
    pipes.forEachAlive(function(p:FlxSprite) { 
        ...
        if (p.ID == 0 && p.x + p.width < bird.x && !iAmDead) { p.ID = 99; score++; addCoins(1); scoreBounce = 1.35; spawnScorePopup(); FlxG.camera.flash(0x08FFFFFF, 0.1); refreshScoreUI(); updateCrown(); checkDifficulty(); if (isMultiplayer) netSend("SCORE:" + score); } 
    });
    pipeCapGroup.forEachAlive(function(c:FlxSprite) { c.velocity.x = pipeSpeed; if (c.x < -120) c.kill(); }); pipeHighlightGroup.forEachAlive(function(h:FlxSprite) { h.velocity.x = pipeSpeed; if (h.x < -120) h.kill(); }); pipeShadowGroup.forEachAlive(function(s:FlxSprite) { s.velocity.x = pipeSpeed; if (s.x < -120) s.kill(); }); powerups.forEachAlive(function(pu) { pu.velocity.x = pipeSpeed; }); powerupGlowGroup.forEachAlive(function(g:FlxSprite) { g.velocity.x = pipeSpeed; if (g.x < -120) g.kill(); }); powerupLabelGroup.forEachAlive(function(l:FlxText) { l.velocity.x = pipeSpeed; if (l.x < -120) l.kill(); });
}

function updateSlingshot(elapsed:Float) {
    if (inCountdown) return;
    if (equippedTrailId != "none" && !iAmDead && bird.velocity.x > 0) { trailTimer += elapsed; if (trailTimer >= 0.05) { trailTimer = 0; spawnTrailParticle(bird); } }

    if (!isDraggingSling && bird.velocity.x == 0 && !iAmDead) {
        if (FlxG.mouse.justPressed) {
            var dx = FlxG.mouse.x - (bird.x + 19); var dy = FlxG.mouse.y - (bird.y + 16);
            if (Math.sqrt(dx*dx + dy*dy) < 50) { isDraggingSling = true; dragStartX = bird.x + 19; dragStartY = bird.y + 16; slingLine.visible = true; }
        }
    }

    if (isDraggingSling) {
        var mx = FlxG.mouse.x; var my = FlxG.mouse.y; var dist = Math.sqrt((mx-dragStartX)*(mx-dragStartX) + (my-dragStartY)*(my-dragStartY)); var maxDrag = 150;
        if (dist > maxDrag) { mx = dragStartX + (mx-dragStartX) * (maxDrag/dist); my = dragStartY + (my-dragStartY) * (maxDrag/dist); }
        bird.x = mx - 19; bird.y = my - 16; var dx = dragStartX - mx; var dy = dragStartY - my; bird.angle = Math.atan2(dy, dx) * 180 / Math.PI;
        slingLine.x = mx; slingLine.y = my; var sldx = dragStartX - mx; var sldy = dragStartY - my; slingLine.scale.set(Math.sqrt(sldx*sldx + sldy*sldy), 4); slingLine.angle = Math.atan2(sldy, sldx) * 180 / Math.PI; slingLine.origin.set(0, 2);

        if (FlxG.mouse.justReleased) { isDraggingSling = false; slingLine.visible = false; bird.velocity.x = dx * 8; bird.velocity.y = dy * 8; bird.acceleration.y = gravity * 0.5; slingshotShotsLeft--; FlxG.camera.flash(0x22FFFFFF, 0.2); powerUsed = false; }
    } else if (bird.velocity.x > 0) {
        var skData = getSkinData(equippedSkinId); if (FlxG.mouse.justPressed && !powerUsed && skData.power != "none") { powerUsed = true; activatePower(skData.power); }
        bird.angle = Math.atan2(bird.velocity.y, bird.velocity.x) * 180 / Math.PI;

        FlxG.overlap(bird, slingshotTargets, function(b, t) { t.kill(); spawnDeathBurst(t); score += 10; addCoins(2); scoreBounce = 1.35; spawnScorePopup(); FlxG.camera.shake(0.01, 0.1); refreshScoreUI(); });

        if (bird.x > FlxG.width + 100 || bird.y > FlxG.height || bird.y < -300) {
            if (slingshotTargets.countLiving() == 0) { generateSlingshotLevel(); } else if (slingshotShotsLeft <= 0) { showGameOver(); } else { resetBirdSlingshot(); }
        }
    }
}

function activatePower(pow:String) {
    FlxG.camera.flash(0x55FFFFFF, 0.3); spawnVFX(bird, 0xFFFFFFFF);
    if (pow == "speed") { bird.velocity.x *= 1.8; bird.velocity.y *= 1.8; } else if (pow == "heavy") { bird.acceleration.y = gravity * 1.5; bird.velocity.y += 200; birdSquash = 1.5; } else if (pow == "explode") {
        if (gameState == "SLINGSHOT") { slingshotTargets.forEachAlive(function(t:FlxSprite) { var dx = t.x - bird.x; var dy = t.y - bird.y; if (Math.sqrt(dx*dx + dy*dy) < 180) { t.kill(); spawnDeathBurst(t); score+=10; addCoins(2); } }); } else { pipes.forEachAlive(function(p:FlxSprite) { var dx = p.x - bird.x; var dy = p.y - bird.y; if (Math.sqrt(dx*dx + dy*dy) < 250) p.kill(); }); }
    } else if (pow == "ghost") { isGhost = true; bird.alpha = 0.5; new FlxTimer().start(2, function(t) { isGhost = false; bird.alpha = 1; }); }
}

function spawnScorePopup() { 
    var pop = new FlxText(Std.int(bird.x + 20), Std.int(bird.y - 30), 100, "+1", 22); pop.setFormat(Paths.font(currentFont), 22, 0xFFFFEE00, "center", 2, 0xFF000000); pop.cameras = [uiCam]; scorePopGroup.add(pop); FlxTween.tween(pop, {y: pop.y - 60, alpha: 0}, 0.7, {ease: FlxEase.quadOut, onComplete: function(_) { pop.destroy(); }});
}
...
function spawnTrailParticle(obj:FlxSprite) {
    ...
    var trailData = getTrailData(equippedTrailId); var col:Int = trailData.color; var dur = 0.25 + FlxG.random.float(0, 0.15); var vx = FlxG.random.float(-30, -70); var vy = FlxG.random.float(-15, 15); var shrinks = true;
    ...
    p.makeGraphic(size, size, col); p.reset(obj.x + 19 - size / 2 + FlxG.random.float(-5, 5), obj.y + 16 - size / 2 + FlxG.random.float(-5, 5)); p.alpha = 0.5; p.velocity.set(vx, vy); trailGroup.add(p);
    if (shrinks) FlxTween.tween(p, {alpha: 0, "scale.x": 0.1, "scale.y": 0.1}, dur, { onComplete: function(twn) { p.kill(); } }); else FlxTween.tween(p, {alpha: 0}, dur, { onComplete: function(twn) { p.kill(); } });
}

function spawnPipe() {
    var pGap = pipeGap; if (gameMode == 7) pGap = FlxG.random.float(120, 300);
    var pipeY = FlxG.random.float(100, FlxG.height - pGap - 140);
    var col = getPipeColor(); var darkCol = darkenColor(col, 0.6); var lightCol = lightenColor(col, 1.4); 
    ...
    var botCap = pipeCapGroup.recycle(FlxSprite); if (botCap == null) botCap = new FlxSprite(); botCap.makeGraphic(capW, capH, darkCol); botCap.reset(FlxG.width - (capW - pipeW) / 2, pipeY + pGap); pipeCapGroup.add(botCap);
    var botHL = pipeHighlightGroup.recycle(FlxSprite); if (botHL == null) botHL = new FlxSprite(); botHL.makeGraphic(8, FlxG.height, lightCol); botHL.reset(FlxG.width + 6, pipeY + pGap); botHL.alpha = 0.3; pipeHighlightGroup.add(botHL);
    var botSH = pipeShadowGroup.recycle(FlxSprite); if (botSH == null) botSH = new FlxSprite(); botSH.makeGraphic(10, FlxG.height, darkCol); botSH.reset(FlxG.width + pipeW - 10, pipeY + pGap); botSH.alpha = 0.35; pipeShadowGroup.add(botSH);
...
function collectPowerup(pu:FlxSprite) {
    switch(pu.ID) { 
        ...
        case 2: hasShield = true; bird.alpha = 0.5; addCoins(5); new FlxTimer().start(5, function(t) { hasShield = false; bird.alpha = 1; }); 
        case 3: pipeSpeed = -650; addCoins(5); new FlxTimer().start(5, function(t) { pipeSpeed = -300 - ((currentLevel - 1) * 35); }); 
...
function killBird() { 
    if (iAmDead) return; iAmDead = true; bird.velocity.x = pipeSpeed; bird.acceleration.y = gameMode == 6 ? -gravity : gravity; bird.color = 0xFF444444; birdWing.color = 0xFF333333; birdHat1.color = 0xFF555555; birdHat2.color = 0xFF555555; spawnDeathBurst(bird); FlxG.camera.shake(0.025, 0.35); FlxG.camera.flash(0x66FF0000, 0.35); addCoins(10); 
    if (isMultiplayer) { netSend("DEAD"); netSend("SUBMIT_SCORE:" + score); checkBattleRoyaleEnd(); } else { saveToLeaderboard(score, false); showGameOver(); }
}

function checkBattleRoyaleEnd() { 
    var aliveCount = 0; for (i in 0...activePlayers.length) { if (!Reflect.field(deadMap, activePlayers[i])) aliveCount++; } if (!iAmDead) aliveCount++;
    if (aliveCount <= 1 && (activePlayers.length > 0 || iAmDead)) { if (!iAmDead) { currentLeader = myNickname; addCoins(25); netSend("SUBMIT_SCORE:" + score); saveToLeaderboard(score, true); } showGameOver(); } else if (iAmDead) { lobbyText.text = "SPECTATING... " + aliveCount + " LEFT"; lobbyText.visible = true; } 
}

function showGameOver() {
    if (gameState == "DEAD") return; gameState = "DEAD"; playMusicTrack(""); if (pipeTimer != null) pipeTimer.cancel(); pipes.forEach(function(p) { p.velocity.x = 0; }); pipeCapGroup.forEach(function(c) { c.velocity.x = 0; }); pipeHighlightGroup.forEach(function(h) { h.velocity.x = 0; }); pipeShadowGroup.forEach(function(s) { s.velocity.x = 0; });
    ...
    var overText = new FlxText(0, 0, FlxG.width, result, 64); overText.setFormat(Paths.font(currentFont), 64, col, "center", 4, 0xFF000000); overText.screenCenter(); overText.y -= 70; overText.cameras = [uiCam]; overText.alpha = 0; overText.scale.set(2.5, 2.5); add(overText); FlxTween.tween(overText, {alpha: 1}, 0.4, {ease: FlxEase.quadOut}); FlxTween.tween(overText.scale, {x: 1, y: 1}, 0.5, {ease: FlxEase.elasticOut});
    var coinSummary = "  |  +" + coinEarnedThisRound + " FLOCKERS"; var finalScore = isMultiplayer ? "SCORE: " + score + "  |  " + (won ? "#1 WINNER" : "ELIMINATED") + coinSummary : "SCORE: " + score + coinSummary;
    var scoreInfo = new FlxText(0, 0, FlxG.width, finalScore + "\n\n[ENTER] RETRY", 22); scoreInfo.setFormat(Paths.font(currentFont), 22, 0xFFCCCCCC, "center", 2, 0xFF000000); scoreInfo.screenCenter(); scoreInfo.y += 20; scoreInfo.cameras = [uiCam]; scoreInfo.alpha = 0; add(scoreInfo); FlxTween.tween(scoreInfo, {alpha: 1}, 0.5, {startDelay: 0.4, ease: FlxEase.quadOut});
}

function resetGameplay() { 
    score = 0; currentLevel = 1; coinEarnedThisRound = 0; netSendAccum = 0; trailTimer = 0; hasShield = false; isGhost = false; powerUsed = false;
    pipeSpeed = -300; pipeGap = 230; pipeInterval = 1.6; gravity = 1500; jumpForce = -500;
    if (gameMode == 2) { pipeSpeed = -450; pipeGap = 160; } else if (gameMode == 4) { pipeSpeed = -700; pipeInterval = 0.8; } else if (gameMode == 5) { gravity = 700; jumpForce = -350; } else if (gameMode == 6) { gravity = -1500; jumpForce = 500; }
    bgPropGroup.clear(); for (i in 0...10) spawnBGProp(true);
}
...
function startSlingshot() {
    ...
    titleText.visible = false; subtitleText.visible = false; lobbyText.visible = false; typingText.visible = false; typingBg.visible = false; scoreText.visible = true; scoreShadow.visible = true; levelText.visible = true; bird.visible = true; birdWing.visible = true; birdEye.visible = true; birdBeak.visible = true; birdHat1.visible = true; birdHat2.visible = true; birdNickTag.visible = true;
    var sDat = getSkinData(equippedSkinId); bird.color = sDat.color; birdHat1.color = 0xFFFFFFFF; birdHat2.color = 0xFFFFFFFF; birdWing.color = darkenColor(sDat.color, 0.7); birdNickTag.text = myNickname;
    levelText.text = "SLINGSHOT MODE"; generateSlingshotLevel();
}

function resetBirdSlingshot() { bird.acceleration.y = 0; bird.velocity.set(0,0); bird.x = 150; bird.y = FlxG.height - 150; bird.angle = 0; isDraggingSling = false; powerUsed = false; if (slingshotShotsLeft <= 0) showGameOver(); }

function generateSlingshotLevel() {
    slingshotTargets.clear(); slingshotShotsLeft = 3; currentLevel++; var numTargets = 3 + currentLevel; var startX = FlxG.width * 0.6;
    for(i in 0...numTargets) { var tx = startX + FlxG.random.float(0, FlxG.width * 0.3); var ty = FlxG.height - 30 - FlxG.random.float(40, FlxG.height * 0.6); var t = new FlxSprite(tx, ty).makeGraphic(40, 40, FlxG.random.color(0xFF555555, 0xFFFFFFFF)); slingshotTargets.add(t); }
    resetBirdSlingshot();
}

function beginMultiplayer() {
    lobbySlotGroup.clear(); lobbyBgGroup.clear(); lobbyRoomText.visible = false; refreshPlayerSidebar(); inCountdown = true; var count = 3;
    var t = new FlxText(0, 0, FlxG.width, "3", 140); t.setFormat(Paths.font(currentFont), 140, 0xFFFFFFFF, "center", 6, 0xFF000000); t.screenCenter(); t.cameras = [uiCam]; add(t); playMusicTrack("racingTillDawn");
    new FlxTimer().start(1, function(tmr) { 
        count--; if (count <= 0) { 
            tmr.cancel(); t.text = "GO!"; t.color = 0xFF00FF88; t.scale.set(1.5, 1.5); FlxTween.tween(t, {alpha: 0}, 0.5, {ease: FlxEase.quadIn, onComplete: function(_) { t.destroy(); }}); FlxG.camera.flash(0x33FFFFFF, 0.3); inCountdown = false; iAmDead = false; resetGameplay(); enterPlayState(); restartPipeTimer(); if (isMultiplayer) netSend("CHAT:SKIN:" + equippedSkinId);
        } else { 
            t.text = Std.string(count); t.scale.set(1.5, 1.5); FlxTween.tween(t.scale, {x: 1, y: 1}, 0.4, {ease: FlxEase.elasticOut}); t.color = [0xFFFFEE00, 0xFFFF9900, 0xFFFF4444][3 - count]; FlxG.camera.shake(0.004, 0.1); 
        } 
    }, 0);
}

function enterPlayState() {
    gameState = "PLAYING"; titleText.visible = false; subtitleText.visible = false; lobbyText.visible = false; typingText.visible = false; typingBg.visible = false; scoreText.visible = true; scoreShadow.visible = true; levelText.visible = true; bird.visible = true; birdWing.visible = true; birdEye.visible = true; birdBeak.visible = true; birdHat1.visible = true; birdHat2.visible = true; birdNickTag.visible = true; bird.acceleration.y = gameMode == 6 ? -gravity : gravity; 
    var sDat = getSkinData(equippedSkinId); if (gameMode == 3 && tagger == myNickname) bird.color = 0xAAFF0000; else bird.color = sDat.color; birdHat1.color = 0xFFFFFFFF; birdHat2.color = 0xFFFFFFFF; birdWing.color = darkenColor(sDat.color, 0.7); bird.y = FlxG.height / 2; bird.velocity.y = 0; birdNickTag.text = myNickname;
}
...
function renderLeaderboardLocal() {
    leaderboardGroup.clear(); var entries = (lbCategory == 0) ? lbSolo : lbMulti; var startY = FlxG.height * 0.28; var maxDisplay = 6; var catStr = (lbCategory == 0) ? "SOLO" : "MULTIPLAYER"; lobbyText.text = catStr + " LEADERBOARD  [<-/->] SWITCH  [UP/DOWN] SCROLL  [R] RESET  [ESC] BACK";
    if (entries == null || entries.length == 0) { var empty = new FlxText(0, startY + 50, FlxG.width, "NO SCORES YET", 32); empty.setFormat(Paths.font(currentFont), 32, 0xFF666666, "center", 2, 0xFF000000); empty.cameras = [uiCam]; leaderboardGroup.add(empty); } 
    else { 
        var medalColors = [0xFFFFD700, 0xFFC0C0C0, 0xFFCD7F32]; var medalIcons = ["1st", "2nd", "3rd"]; var iterLimit = Std.int(Math.min(maxDisplay, entries.length - lbScroll));
        for (i in 0...iterLimit) { 
            var realIdx = lbScroll + i; var col = realIdx < 3 ? medalColors[realIdx] : 0xFFAAAAAA; var prefix = realIdx < 3 ? medalIcons[realIdx] + "  " : (realIdx + 1) + ". "; var rowBg = new FlxSprite(Std.int(FlxG.width * 0.15), Std.int(startY + (i * 42) - 4)).makeGraphic(Std.int(FlxG.width * 0.7), 36, realIdx < 3 ? 0xFF1A1A2E : 0xFF101018); rowBg.alpha = 0.5; rowBg.cameras = [uiCam]; leaderboardGroup.add(rowBg); 
            var entry = new FlxText(0, startY + (i * 42), FlxG.width, prefix + entries[realIdx].name + "    " + entries[realIdx].score, 24); entry.setFormat(Paths.font(currentFont), 24, col, "center", 2, 0xFF000000); entry.cameras = [uiCam]; entry.alpha = 0; FlxTween.tween(entry, {alpha: 1}, 0.3, {startDelay: i * 0.06, ease: FlxEase.quadOut}); leaderboardGroup.add(entry); 
        } 
    }
}
